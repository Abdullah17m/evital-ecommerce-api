...
class OrderService {
    ...
    // async createUserOrder(order: Order) {
    //     try {
    //         await pool.query("BEGIN");
    ...
    //         if (!cartData.rowCount) return formatResponse(true, "Cart not found or does not belong to user", {});

    //         let total_amount = Number(cartData.rows[0].total_amount);
    ...
    //             `SELECT ci.product_id, ci.quantity, p.price FROM cartitems ci 
    //              JOIN products p ON ci.product_id = p.product_id WHERE ci.cart_id = $1`,
    //             [cart_id]
    //         );

    //         if (!cartItems.rowCount) return formatResponse(true, "No items in cart");

    //         for (const { product_id, quantity, price } of cartItems.rows) {
